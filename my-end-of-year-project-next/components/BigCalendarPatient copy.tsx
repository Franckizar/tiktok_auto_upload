...
const getPatientIdFromToken = (token: string | null): number | null => {
  ...
  try {
    ...
    return decodedPayload.patient_id || null;
  } catch (error) {
...
const BigCalendarPatient = () => {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        ...
        console.log("📌 Extracted Patient ID from JWT:", patientId);

        if (!patientId) throw new Error("Patient ID not found in token.");

        const response = await fetch(`https://wambs-clinic.onrender.com/api/v1/auth/appointments/patient/${patientId}`, {
          method: "GET",
          headers: {
        ...
        const mappedEvents: AppointmentEvent[] = data.map((appt) => ({
          id: appt.id,
          title: appt.title || appt.reason || "Appointment",
          start: new Date(appt.startTime), // <-- FIXED
          end: new Date(appt.endTime),     // <-- FIXED
          status: appt.status,
          reason: appt.reason,
        }));
        ...
        setError(error instanceof Error ? error.message : "Unknown error loading appointments.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const calendarFormats: Partial<Formats> = {
    dayHeaderFormat: "ddd",
    ...
    <>
      ...
      <div className="h-screen w-screen overflow-hidden">
        <Calendar
          ...
          startAccessor="start"
          endAccessor="end"
          views={[Views.WORK_WEEK, Views.DAY, Views.WEEK]}
          view={view}
          ...
          popup={false}
          tooltipAccessor={(event) => `${event.title} - ${event.reason}`}
          onSelectEvent={(event) => {
