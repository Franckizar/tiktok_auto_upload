...
import { subjectsData as rawSubjectsData } from '@/lib/data'
...
type RawSubject = {
  ...
  name: string;
  teachers: string[];
  // info missing in raw, so we add it below
};

// Final, type-safe Subject type
type Subject = {
  id: number;
  info: string;
  name: string;
...
// Map and add info field for each subject
const subjectsData: Subject[] = rawSubjectsData.map((item: RawSubject) => ({
  ...item,
  info: `${item.name} (${item.teachers.join(", ")})`,
}));
