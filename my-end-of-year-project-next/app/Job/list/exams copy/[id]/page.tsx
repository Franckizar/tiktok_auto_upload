...
const UserAppointmentListPage = () => {
  const params = useParams();
  const userId = params?.userId; // expects route: /list/appoint/user/[userId]  here not not for for the id
  ...
  useEffect(() => {
    ...
    if (!userId) return;
    fetch(`https://wambs-clinic.onrender.com/api/v1/auth/appoint/user/${userId}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
      ...
      <div className="flex items-center justify-between">
        <h1 className="hidden md:block text-lg font-semibold">User&apos;s Appointments</h1>
...
export default UserAppointmentListPage;
